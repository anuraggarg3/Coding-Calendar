$(document).ready(function() {
  ...
  .then(data => {
    ...
    for(var i = 0; i < sites.length; ++i) {
      site = sites[i];
      // console.log(site[0])
      if(site[0]=='HackerRank') continue;
      if(site[0]=='HackerEarth') continue;
      if(site[0]=='Kick Start') continue;
      var site_contests = [];
...
function addPill(site, contests_num, index) {
  ...
  .appendTo(".nav-pills");
  // $('.nav-pills').append('<img id="theImg" src="logo.png" height="30px"/>')
// console.log(site[0])
}
...
function constructTablesBodies(site_contests) {
  ...
  for(var j = 0; j < site_contests.length; ++j) {
    ...
    if(contest["status"] == "BEFORE") {
      ...
      add_to_calendar = "<a class=\"add-to-calendar\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Add to Calendar\" target=\"_blank\" href=\"" + add_to_calendar + "\"><i class=\"far fa-calendar-alt\" style=\"font-size:30px;color:white\"></i></a> &nbsp;";
      temp_time=add_to_calendar
    }
    temp_text += "<a href=" + contest["url"] + ">" + contest["name"] + "</a></td><td>" + localTimeFromUtc(contest["start_time"]) + "</td><td>" + durationToText(contest["duration"]) + "</td><td>"+ temp_time+ "</td></tr>";
...
function constructTable(contests_num, table_tbody, table_type) {
  table_thead = "<thead class=\"border-top border-bottom\"><tr><th>Contest Name</th><th>Start Date <br>Time</th><th>Duration</th><th>Reminder</th></tr></thead>";
var table = $("<div>", {
    "class": table_type,
    "html": $("<div>", {
...
function localTimeFromUtc(utcTime) {
  ...
  var localDateString = DateFormat.format.date(givenDate, 'dd  MMM  yyyy <br> HH:mm');
